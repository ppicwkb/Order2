    ...
    try {
        ...
        function populateFilters() {
            ...
            const POs = [...new Set(stockData.map(row => row[5]).filter(item => item && item !== '-'))];
            ...
            if (POFilter) {
                POFilter.innerHTML = '<option value="">Semua PO</option>' + 
                    POs.map(k => `<option value="${k}">${k}</option>`).join('');
            }
        ...
        function exportSizePackingToPDF() {
            try {
                ...
                allSizePackingSummaryData.forEach(([key, data], index) => {
                    ...
                    doc.text(`Total MC: ${data.items} | Total Kg: ${data.kg} | No PO: ${data.PO}`, 25, yPosition);
                    yPosition += 10;
        ...
        function applyFilters() {
            ...
            filteredData = stockData.filter(row => {
                ...
                return matchesDate && matchesProduct && matchesBrand && matchesPO && matchesLocation && matchesSearch;
            });
        ...
        function resetFilters() {
            ...
            const POFilterEl = document.getElementById('POFilter');
            ...
            if (POFilterEl) POFilterEl.value = '';
